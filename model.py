...
class GAN:
    ...
    def build_model(self):
        ...
        img_height = 7
        num_channels = 1
        t_real_image = tf.placeholder('float32', [self.options['batch_size'],img_height, img_size, num_channels], name = 'real_image')
        t_wrong_image = tf.placeholder('float32', [self.options['batch_size'],img_height, img_size, num_channels], name = 'wrong_image')
    ...
    def discriminator(self, image, t_text_embedding, reuse=False):
        ...
        print ('check')
        print (image.shape)
        h0 = ops.lrelu(ops.conv2d(image, self.options['df_dim'] * 4, name = 'd_h0_conv')) #32
        print (h0.shape)
